...
class clsModelData(object):
    ...
    def ReloadSpecs(self, Node):
        ...
        self.output.Date = [datetime.datetime.strptime(x, '%Y,%m,%d').date() for x in json.load(open(Datepath))]
    ...
    def DivideData(self):
        ...
        Qbins = deepcopy([float(x) for x in self.Capacity.Qlist])
    ...
    def LoadNodeOutput(self, variablelist):
        ...
        for variable in variablelist:
            ...
            for row in dfile:
                ...
                values = [float(x) for x in splitset[1:]]                
